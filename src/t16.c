...
int threeSumClosest(int* nums, int numsSize, int target) {
	int retVal = INT_MAX, curVal, tmpVal;
	int i,j,k;
	...
	for (i = 0; i < numsSize - 2; i++)
	{
		...
		{
			tmpVal = nums[i] + nums[j];
			for (k = j + 1; k < numsSize; k++)
			{
				curVal = tmpVal+ nums[k];
				curDistence = abs(curVal - target);
				if (curDistence < minDistence)
				{
					minDistence = curDistence;
					retVal = curVal;
					if (!minDistence) return retVal;
				}
			}
		}
	}
